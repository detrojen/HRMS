...
const GameSlotBook = () => {
  ...
  const interestedEmployees = interestedEmployeesQueryData?.data.data
  const slots = slotQueryData?.data.data
          ...
          gameId === null ?
            ...
            <>
              ...
              <div className="flex flex-row gap-2 flex-wrap">
                {slots && slots.map(slot => <div key={slot.id} className={`w-fit p-2 ${selectedSlot == slot.id ? "selected-slot" : getSlotAppearnceClass(slot.available)}`} onClick={() => { setSelectedSlot(slot.id) }}>{slot.startsFrom}</div>)}
              </div>
