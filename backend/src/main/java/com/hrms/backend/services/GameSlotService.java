...
import com.hrms.backend.repositories.GameSlotRepository;
...
public class GameSlotService {
    ...
    public List<GameSlotResponseDto> getGameSlotsByDate(Long gameTypeId,Date slotDate){
    ...
    public GameSlotResponseDto getSlotById(Long slotId){
        GameSlot slot = this.gameSlotRepository.findById(slotId).orElseThrow(()-> new RuntimeException("slot not found"));
